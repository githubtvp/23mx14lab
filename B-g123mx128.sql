...
select * from tab;
desc employee;
select rowid, rownum,empno,fname, job, salary from 
(select * from employee order by salary desc) where rownum<=5;
create table sam (sid number(2),
name char(5));
insert into sam values(&sid, '&name');
commit;
select rowid,sid,name from sam;
update sam set name='ram' where name='Ram';
delete from sam where rowid='AAAuBwAABAAAvXhAAF';
-----------------------
